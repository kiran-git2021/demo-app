...
import { Area, AreaChart, CartesianGrid, XAxis, ReferenceArea } from "recharts"
...
const chartConfig = {
  Parameter1: { label: "Parameter1", color: "var(--chart-1)" },
  Parameter2: { label: "Parameter2", color: "var(--chart-2)" },
}

export function ChartAreaInteractive() {
  const [activeChart, setActiveChart] = useState("Parameter1")
  const [timeRange, setTimeRange] = useState("90d")

  // Zoom state
  const [zoomArea, setZoomArea] = useState(null)
  const [isZooming, setIsZooming] = useState(false)
  const [zoomDomain, setZoomDomain] = useState({ left: 0, right: chartData.length - 1 })
  const [originalDomain] = useState({ left: 0, right: chartData.length - 1 })
  ...
  const visibleData = filteredData.slice(zoomDomain.left, zoomDomain.right + 1)
  ...
  const handleMouseDown = (e) => {
    if (e && e.activeLabel) {
      ...
      const index = filteredData.findIndex((item) => item.date === e.activeLabel)
  ...
  const zoomIn = () => {
    ...
    const newRight = Math.min(filteredData.length - 1, Math.ceil(center + newRange / 2))
  ...
  const zoomOut = () => {
    const range = zoomDomain.right - zoomDomain.left
    const center = (zoomDomain.left + zoomDomain.right) / 2
    const newRange = Math.min(filteredData.length - 1, range * 1.3)
    const newLeft = Math.max(0, Math.floor(center - newRange / 2))
    const newRight = Math.min(filteredData.length - 1, Math.ceil(center + newRange / 2))
    setZoomDomain({ left: newLeft, right: newRight })
  }

  const resetZoom = () => setZoomDomain(originalDomain)
    ...
    <Card className="pt-0">
      <CardHeader className="flex items-center gap-2 space-y-0 border-b py-5 sm:flex-row">
        <div className="grid flex-1 gap-1">
          ...
          <CardDescription>
            Showing data history for the selected time range. Click and drag to zoom.
          </CardDescription>
        </div>
        ...
        <div className="flex gap-2 items-center">
          ...
          {/* Zoom Buttons */}
          <Button variant="outline" size="sm" onClick={zoomIn} disabled={zoomDomain.right - zoomDomain.left <= 1}><ZoomIn className="h-4 w-4"/></Button>
          <Button variant="outline" size="sm" onClick={zoomOut} disabled={zoomDomain.left === originalDomain.left && zoomDomain.right === originalDomain.right}><ZoomOut className="h-4 w-4"/></Button>
          <Button variant="outline" size="sm" onClick={resetZoom} disabled={zoomDomain.left === originalDomain.left && zoomDomain.right === originalDomain.right}><RotateCcw className="h-4 w-4"/></Button>
        </div>
      </CardHeader>
      ...
      <CardContent className="px-2 pt-4 sm:px-6 sm:pt-6">
        <ChartContainer config={chartConfig} className="aspect-auto h-[250px] w-full">
          ...
          >
            <defs>
              <linearGradient id="fillParameter1" x1="0" y1="0" x2="0" y2="1">
                <stop offset="5%" stopColor="var(--color-Parameter1)" stopOpacity={0.8}/>
                <stop offset="95%" stopColor="var(--color-Parameter1)" stopOpacity={0.1}/>
              </linearGradient>
              <linearGradient id="fillParameter2" x1="0" y1="0" x2="0" y2="1">
                <stop offset="5%" stopColor="var(--color-Parameter2)" stopOpacity={0.8}/>
                <stop offset="95%" stopColor="var(--color-Parameter2)" stopOpacity={0.1}/>
              </linearGradient>
            </defs>
            ...
            <Area dataKey="Parameter1" type="natural" fill="url(#fillParameter1)" stroke="var(--color-Parameter1)" stackId="a"/>
            <Area dataKey="Parameter2" type="natural" fill="url(#fillParameter2)" stroke="var(--color-Parameter2)" stackId="a"/>
            {zoomArea && (
              <ReferenceArea
                x1={filteredData[zoomArea.left]?.date}
                x2={filteredData[zoomArea.right]?.date}
                strokeOpacity={0.3}
                fillOpacity={0.1}
              />
            )}
